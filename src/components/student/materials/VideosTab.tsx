...
interface Props { videos: Material[] }

export function VideosTab({ videos }: Props) {
  return (
    <div className="space-y-4">
      {videos.map((video) => {
        const fecha = new Date(video.uploaded_at)
          ...
          <Card key={video.id} className="rounded-[16px]">
            <div className="flex flex-col md:flex-row">
              ...
              <div className="flex-1">
                <h3 className="text-[16px] font-medium text-[#333]">{video.title}</h3>
                ...
                <div className="flex items-center justify-between mt-4 text-[13px] text-[#A0A0A0]">
                  ...
                  <Button
                    className="bg-[#4E81BD] hover:bg-[#4E81BD]/90 text-white"
                    asChild
                  >
