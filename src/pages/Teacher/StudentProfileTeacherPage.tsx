...
import { useStudentStudies, Study } from "../../hooks/teacher/useStudentStudies";
...
export default function StudentProfileTeacherPage({ mode }: Props) {
  ...
  if (mode === "view") {
    ...
    if (studentsError) return <p className="p-8 text-red-500">Error: {studentsError}</p>;
    ...
    return (
      <div className="p-8">
        ...
        {!studiesLoading && !studiesError && studies.length === 0 && <p>No hay estudios para este estudiante.</p>}
        {!studiesLoading && !studiesError && studies.map((st: Study) => (
          <Card key={st.id} className="p-4 mb-2">
            <h3 className="font-medium">{st.title}</h3>
            <p className="text-sm text-gray-500">Protocolo: {st.protocol || "—"}</p>
